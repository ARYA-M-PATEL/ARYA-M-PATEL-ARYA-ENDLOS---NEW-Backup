...
class Seo extends Admin_Controller  {
	...
	public function view()
    {
        ...
        $data['data'] = $this->main->get($this->table, 'name, email, phone_number, msg_subject, message, created_at', ['id' => $id, 'is_deleted' => 0]);
    ...
    public function create_update()
	{
        if($this->input->is_ajax_request()) {
            ...
            if($id != 0) {
                $data['data'] = $this->main->get($this->table, 'page_name, title, keywords, description', ['id' => $id, 'is_deleted' => 0]);
                if(!$data['data']) {
                    flashMsg(0, "", "$this->title not found.", $this->redirect);
                }
            }
	...
	public function delete()
    {
        ...
        if ($this->form_validation->run() == FALSE)
            ...
            if(!$this->main->get($this->table, 'id', ['id' => $id])) responseMsg(0, "", "$this->title not found.");
